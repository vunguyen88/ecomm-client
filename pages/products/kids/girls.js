...
const KidGirlsProducts = ({products}) => {
...
KidGirlsProducts.getInitialProps = async (context, client) => {
    console.log('CLIENT IN IN PRODUCTDEX ', client)
    console.log('GET INITIAL CALL FROM PRODUCTS PAGE ')
    try {
        const { data } = await client.get('/api/products');
        const menProducts = data.filter(product => product.category.includes('kids-girl'))
...
export default KidGirlsProducts;